...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!fbRes.ok) {
      console.error('Facebook API error (get reel status):', fbData.error);
      return NextResponse.json(
        { message: fbData?.error?.message || 'Failed to get reel status from Facebook.' },
        { status: 500 }
      );
    }
    ...
    console.error('Error getting reel status:', err);
